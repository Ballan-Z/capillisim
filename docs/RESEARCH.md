...
Compiled 2026-07-02 (web research). Two tracks: downloadable cap-image datasets we
can import to grow the colour inventory (`caps.db`), and photomosaic/assembly
techniques worth adopting. Feeds the dithering (`core/dither.py`), inventory gap
report, and projector stencil/per-colour work.
...
5. **Kaggle: bottle cap classification (tahuuanh)**, https://www.kaggle.com/datasets/tahuuanh/bottle-cap-classification; brand-classification set; Kaggle license (check page); auth: Kaggle account + `kaggle` API token (our `.env` has `KAGGLE_KEY`; still needs `KAGGLE_USERNAME`); how we'd use it: brand-organised folders group caps, so extract one representative colour per brand and tag `caps.db` rows with a brand hint alongside the colour.
