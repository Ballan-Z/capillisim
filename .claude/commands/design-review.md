...
**Capillisim environment note:** Playwright MCP is not installed on this rig —
run the same phases with the claude-in-chrome tools instead (synthetic
events/DOM reads via `javascript_tool`; Chrome may be minimized under remote
control, so avoid coordinate clicks and CDP screenshots — see the
`webapp-verify-environment` memory). The webapp lives at http://localhost:8000
(restart a detached instance after server.py changes).
